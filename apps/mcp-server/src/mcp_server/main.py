"""Minimal stdlib HTTP service for the MCP server.

Stand-in until the real MCP server exists: that one must speak streamable
HTTP and target the stateless Model Context Protocol spec >= 2026-07-28, so
any replica can serve any request. For now this only answers the health
routes, which is enough to exercise the image, CI and the k8s manifests.
"""
...
SERVICE_NAME: Final[str] = "mcp-server"
