"""Unit tests for the API service. No sockets: the routing and config
helpers are pure functions, so they are tested directly."""
...
from api.main import DEFAULT_PORT, build_response, read_port
...
def test_ok_routes_return_service_status(path: str) -> None:
    ...
    assert status == 200
    assert json.loads(body) == {"service": "api", "status": "ok"}
